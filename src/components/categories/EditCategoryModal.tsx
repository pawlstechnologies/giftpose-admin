...
// ── Types ─────────────────────────────────────────────────────
// 'content' removed from Subcategory — it doesn't exist on the local type
interface Subcategory { id: string; name: string; }
interface Category { id: string; name: string; active: boolean; source: 'manual' | 'ai'; needsReview: boolean; subcategories: Subcategory[]; }
...
export default function EditCategoryModal({ isOpen, category, allCategories, onSubmit, onClose }: EditCategoryModalProps) {
  const [selectedCatId, setSelectedCatId] = useState('');
  const [subInput, setSubInput]           = useState('');
  const [subTags, setSubTags]             = useState<string[]>([]);
  const [contentInput, setContentInput]   = useState('');
  ...
  useEffect(() => {
    if (isOpen && category) {
      setSelectedCatId(category.id);
      setSubTags(category.subcategories.map(s => s.name));
      // 'content' doesn't exist on Subcategory — start with empty array
      setContentTags([]);
      setSubInput('');
      setContentInput('');
    }
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-field">
          <div className="form-label">Parent Category</div>
          <div className="cb-select-wrap">
            <select className="cb-select" value={selectedCatId} onChange={e => setSelectedCatId(e.target.value)}>
              {allCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
        ...
        <button className="cb-btn-submit" onClick={handleSubmit}>Submit</button>
      </div>
    </div>
  );
}