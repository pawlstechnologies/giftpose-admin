...
export default function SubcategoryModal({ isOpen, categories, defaultCategoryId, onSubmit, onClose }: SubcategoryModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 'results' removed — we don't use the return value, just fire and call onSubmit
      await Promise.all(
        finalTags.map(name =>
      ...
      setError(err?.response?.data?.message ?? err?.message ?? 'Failed to create subcategory. Please try again.');
    } finally {
      setLoading(false);
    }
  };
