...
export default function ContentModal({ isOpen, categories, defaultCategoryId, defaultSubId, onSubmit, onClose }: ContentModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message ?? err?.message ?? 'Failed to create content. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-field">
          ...
          <div className="cb-select-wrap">
            ...
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="cb-select-icon"><polyline points="6 9 12 15 18 9" /></svg>
          </div>
        </div>
        ...
        <div className="modal-field">
          ...
          <div className="cb-select-wrap">
            <select className="cb-select" value={selectedSubId} onChange={e => setSelectedSubId(e.target.value)} disabled={subs.length === 0 || loading}>
              {subs.length === 0
        ...
        <button className="cb-btn-submit" onClick={handleSubmit} disabled={!canSubmit}
